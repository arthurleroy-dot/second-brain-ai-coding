...
// Formes minimales du registre (mêmes shapes que /api/entities), déclarées ici
// pour éviter d'importer le module serveur wiki-parser côté client.
...
export default function CandidateCard({
  ...
  // Création : type pré-rempli avec le type suggéré si dispo.
  const [createType, setCreateType] = useState(candidate.suggested_types[0] ?? '');
  const [newType, setNewType] = useState('');

  // Union des types connus + suggérés (au cas où un type suggéré manque au registre).
  const typeOptions = [
    ...types,
    ...candidate.suggested_types
      .filter((t) => !types.some((x) => x.slug === t))
      .map((t) => ({ slug: t, label: entityTypeLabel(t) })),
  ];
  ...
  async function resolve(action: string, extra: Record<string, string> = {}) {
    ...
    setError(null);
    try {
      const res = await fetch('/api/candidates/resolve', {
        method: 'POST',
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4">
      ...
      {/* Type suggéré */}
      {candidate.suggested_types.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          <span className="text-gray-400">Type suggéré :</span>
          {candidate.suggested_types.map((t) => (
            <span
              key={t}
              ...
              >
                ...
                {entities.map((e) => (
                  <option key={e.slug} value={e.slug}>
                    {e.label} ({entityTypeLabel(e.entity_type)})
                  </option>
                ))}
              </select>
              <button
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                disabled={
                  busy ||
                  !createType ||
                  (createType === NEW_TYPE && !newType.trim())
                }
                onClick={() =>
                  resolve('create', {
                    entity_type: createType === NEW_TYPE ? newType.trim() : createType,
                  })
                }
                className="rounded-lg bg-gray-900 px-2.5 py-1.5 text-xs font-medium text-white hover:bg-gray-800 disabled:opacity-50"
              >
